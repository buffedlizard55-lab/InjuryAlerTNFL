...
- **Pages configuration:** the repo originally had legacy Pages from main/root. To publish scheduled artifacts, the Pages build source must be changed to **GitHub Actions** (the workflow needs Pages write/id-token permissions). If an organization policy blocks that setting, main/root still publishes the checked archive and explicitly says the live collector has not run, but scheduled data cannot appear on the site until an admin changes the setting.
