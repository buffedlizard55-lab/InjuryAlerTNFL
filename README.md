...
**Website:** [buffedlizard55-lab.github.io/InjuryAlerTNFL/](https://buffedlizard55-lab.github.io/InjuryAlerTNFL/) · [20-entry source audit](docs/source-audit.md)

An independent, mobile-friendly NFL scoreboard and injury alert prototype. It separates **what was observed**, **what happened in that game**, and **later reported availability**. Each of the 20 archive entries links short evidence excerpts to NFL.com or official club reporting. Eight irregularities are linked in a separate review queue (six held, two annotated). The archive was checked on **September 23, 2026**; it is *not* a complete injury census or a live medical status database.
...
- Browser notifications (optional) work only with the page open and permission granted. They fire only for *new* automatically matched reports from games started within 36 hours; the initial archive does not cause a notification. RSS (`feed.xml`) contains automatic reports, not the historical archive.
...
- `data/archive.json` — 20 game-specific source-checked examples, each with dated claims and official league/team links. `data/review.json` — eight separately flagged source issues. Tracked `data/live.json` and `data/scoreboard.json` are honest *not yet checked* fallbacks for first deployment.
...
- **No automated longitudinal prognosis.** The collector presently handles explicit game outcomes in NFL.com in-game roundups. Individual team statements, TV injury reports, local beat reporters and later clearances need separately validated adapters with entity resolution and conflict review before automatic publication. There is no clinically valid automated severity estimate. *Next session:* integrate official club newsroom/game-day status feeds, source timestamps, and a persistent audited update ledger; add real-event fixture tests and discrepancy monitoring across sources.
...
- **Pages configuration:** GitHub currently reports a *legacy main/root* Pages source. Nonetheless, the `deploy-pages` Action **successfully published** a generated scores/collector artifact after the first merge, so scheduled artifact deployment works in this repository. The integration cannot change the Pages build setting via API (`403 Resource not accessible by integration`). There may be a publishing race after future main commits: the legacy build can temporarily show the tracked, explicitly *not checked* fallback until the next successful Actions deploy. A repository administrator should ideally select **GitHub Actions** as the sole Pages source, but no such setting is required to view the already deployed site.
- **Research scope:** exactly 20 selected, distinct 2026 in-game events, not all NFL injuries. Even sourced stories can later be corrected. Old claims remain a historical record with source dates, not a current health assertion. A human can inspect every link in the audit; no user entry is required.
